...
if __name__ == "__main__":
    ...
    if test_bugs_type == "":
        ...
        with open(Path(OUTPUT_DIR, f"{test_bugs_type}_result.csv"), 'r') as f:
            reader = csv.reader(f)
            for row in reader:
                tmp_lst.append(row)
    ...
    if test_bugs_type == "":
        ...
        FileUtil.dump_json(Path(METRICS_DIR, tool_name, f"{test_bugs_type}_metrics.json"), output)
